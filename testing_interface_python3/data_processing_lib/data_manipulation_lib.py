...
class pni_data_frame :
    ...
    def calc_offset(self) :
        ...
        #self.offset = {0, 0 ,0} #alternative hardcode method
...
def pni_file_decode_sean(fn, os, cc, mn) :
    ...
    sf = float((1000/(0.3671 * cc + 1.5)) / os) #current accepted method
    #sf = float(1000/(0.3671 * cc * os + 1.5)) #alternative
    po = pni_data_frame()
...
def fix_pni_dataset(t, d, f):
    ...
    if vt and ot :
        for i,j in enumerate(zdf) :
            if j != np.nan and j > 4 : #outliers are > 4 std
                print("Removed data point (outlier) " + str(round(d[i],3)) + " at index " + str(i))
    ...
    if it : #interpolate 
        #if ct:
        #    nsn, xx = nan_helper(d[cp+1:cl-cp].to_numpy()) #only interpolate data not cropped 
        #    d[cp+1:cl-cp][nsn]= np.interp(xx(nsn), xx(~nsn), d[cp+1:cl-cp][~nsn])
        #else :
        nsn, xx = nan_helper(d.to_numpy())
        d[nsn]= np.interp(xx(nsn), xx(~nsn), d[~nsn])
