...
def plot_data(f, filename=''):
    ...
    if int(pn, 16) < 1 or int(pn, 16) > 10 :
        print('invalid input')
        plot_data()
    pf(do, sfn, f)
...
def plot_magnetometer_helper(do, n, sfn, f) :
    ...
    if h :
        ...
        ic_fig, ic_axs = plt.subplots(3,1)
        ic_fig.set_size_inches(18.5, 10.5)
        ic_fig.supxlabel('Time (s)')
        ic_fig.supylabel('Magnetic Field (nT)')
        ic_axs[0].set_title('x')
        ic_axs[1].set_title('y')
        ic_axs[2].set_title('z')
        ...
        for i in range(0,3) :
            ...
            if center_on_zero :
                py = do.p.iloc[:,i] - do.p.iloc[:,i].mean()
            ic_axs[i].plot(px, py, label='stdev=' + str(round(py.std(),3)), color=plot_colors[i])
        ...
        #plot single mag
        if a:
            ic_fig.savefig('data_storage/figs/magnetometer_' + str(n) + "_time_series_data_" + str(sfn) + '.png')
        #plot all mags together
        else :
            ic_fig.savefig('data_storage/figs/magnetometer_quad_time_series_data_' + str(sfn) + '.png')
